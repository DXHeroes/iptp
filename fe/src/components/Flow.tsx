...
const Flow: React.FC<Props> = () => {
  ...
  const handleInput = ({ target }: React.ChangeEvent<HTMLInputElement>) =>
    setFlowState((state) => ({ ...state, title: target.value }));
  ...
  return (
    <>
      <div>
        <input
          className="text-20 p-20 bg-greylight w-full mb-40"
          placeholder="Title..."
        ...
        <div className="bg-greylight p-20 rounded-lg mb-40">
          <div className="flex items-center">
            ...
            <button
              className="ml-auto text-blue font-heading"
              onClick={() => setModal(ModalType.CONDITIONS)}
            >
              + Add condition
            </button>
          </div>
              ...
              .map((condition) => (
                ...
                >
                  <div className="bg-greylight px-20 py-10">
                    IF:{' '}
                    <span className="text-green capitalize">{`${condition} ${
                      condition === 'amount'
                        ? ` ${
                            AmountConditionToConditionTypes[
                              flowState.amountCond
                            ]
                          }`
                        : ''
                    }`}</span>
        ...
        <div className="bg-greylight p-20 rounded-lg">
          <div className="flex items-center">
            ...
            <button
              className="ml-auto text-blue font-heading"
              onClick={() => setModal(ModalType.ACTIONS)}
            >
              + Add action
            </button>
          </div>
      ...
      {modal && (
        <div className="fixed bg-black bg-opacity-75 z-10 top-0 left-0 w-full h-full flex">
          <div
            ref={ref}
            className="bg-white max-w-30rem w-full m-auto py-40 px-20 rounded-lg"
          >
