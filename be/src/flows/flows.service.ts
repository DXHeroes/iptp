...
export class FlowsService {
  ...
  async matchTransaction(transactionId: string) {
    ...
    for (const f of flows) {
      ...
      if (f.amount && !this.fulfillsCondition(transaction.tsAmount, f.amount, f.amountCond)) break;
  ...
  private fulfillsCondition(tsAmount: number, flowAmount: number, cond: AmountCondition) {
    switch (cond) {
